...
router.get('/load', async(req, res, next) => {
    try {
        var sqlcount = "select count(*) count from tb_user where is_del=0 ";
        var sql = "select * from tb_user where is_del=0 ";
    ...
    try {
        ...
        if (ids && ids.trim() != "") {
            ...
            if (_.indexOf(ids, user_id + "") != -1) {
                ...
                var sql = 'update tb_user set is_del=1, modified_at=?, modified_id=? where id in (';
                var sql2 = 'delete from tb_user_role where user_id in (';
